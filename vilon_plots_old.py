...
def create_base_vs_lora_comparison(df, in_target_df, cross_target_df):
    ...
    ax1.set_title('In-Target', fontsize=26, fontweight='bold', pad=20)
    ...
    ax1.legend(title='Model Type', loc='best', title_fontsize=18, fontsize=16)
    ...
    ax2.set_title('Cross-Target', fontsize=26, fontweight='bold', pad=20)
    ...
    ax2.tick_params(axis='y', labelsize=0)  # Hide y-axis tick labels
    ax2.set_yticklabels([])  # Remove y-axis tick labels
    ax2.grid(True, alpha=0.3)
    ax2.legend(title='Model Type', title_fontsize=18, fontsize=16, loc='best')
...
def create_prompt_comparison(df, in_target_df, cross_target_df):
    ...
    for i, prompt in enumerate(prompt_order):
        ...
        prompt_melted['phase_experiment'] = (
            ...
            ' (' + prompt_melted['experiment_type'].map({'in_target': 'In-Target', 'cross_target': 'Cross-Target'}) + ')'
        )
        
        sns.violinplot(data=prompt_melted, x='experiment_type', y='score', hue='phase', 
                       ax=ax, inner='quart', palette=['lightcoral', 'skyblue'])

        # Fix x-axis labels to show In-Target and Cross-Target
        ax.set_xticklabels(['In-Target', 'Cross-Target'])
        ...
        if i == 0:
            ax.legend(title='Model Type', labels=['Base', 'LoRA Fine-Tuned'], 
                     title_fontsize=18, fontsize=16, loc='best')
        else:
            ax.legend().remove()
...
def create_model_size_performance_gains(df, in_target_df, cross_target_df):
    ...
    for exp_type, exp_df in [('In-Target', in_target_df), ('Cross-Target', cross_target_df)]:
    ...
    in_target_gains = gains_df[gains_df['experiment_type'] == 'In-Target']
    ...
    cross_target_gains = gains_df[gains_df['experiment_type'] == 'Cross-Target']
...
def create_model_size_performance_scores(df, in_target_df, cross_target_df):
    ...
    in_target_scores = scores_df[scores_df['experiment_type'] == 'In-Target']
    ...
    ax1.tick_params(axis='y', labelsize=22)
    ax1.grid(True, alpha=0.3)
    ax1.legend(title='Model Type', title_fontsize=18, fontsize=16, loc='best')
    
    # Cross-Target Performance Scores
    cross_target_scores = scores_df[scores_df['experiment_type'] == 'Cross-Target']
...
def create_additional_insights_plots(df, in_target_df, cross_target_df):
    ...
    ax1.set_xticklabels(['In-Target', 'Cross-Target'])
