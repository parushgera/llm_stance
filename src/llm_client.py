...
from .prompts import chat_manager, SYSTEM_INSTRUCTION

# Optional: log in to HuggingFace Hub if a token is provided in the
# environment so gated checkpoints (e.g. Llama-3-8B-Instruct) can be
# downloaded. We never ship a token in source.
...
if _HF_TOKEN:
    ...
    except Exception as _exc:  # pragma: no cover - best-effort auth
        print(f"[llm_client] HuggingFace login skipped: {_exc}")


class LLMClient:
    def __init__(self, model_key: str, cache_dir: Optional[str] = None, adapter_path: Optional[str] = None, **kwargs):
        ...
        self.cache_dir = cache_dir if cache_dir is not None else HF_CACHE_DIR
